...
const KiteConnect = function (params) {
  ...
  const routes = {
    'api.login': '/trusted/kiteandroid/user/{user_id}/login',
    'api.twofa': '/trusted/kiteandroid/user/{user_id}/twofa',
  ...
  /**
     * Do the token exchange with the `kite credentials`,
     ...
     * @instance
     *
     * @param {string} user_id Kite user ID.
     * @param {string} password password.
     * @param {string} PIN 2 factor PIN.
     */
  self.createSession = async (user_id, password, pin) => {
    console.log(`createSession - ${user_id} ${password}`)

    const resp = await _post('api.login', {
      user_id: user_id,
      password: password
    })
    console.log(`createSession: ${JSON.stringify(resp)}`)
    if (resp.request_id) {
      // twofa
      const p = await _post('api.twofa', {
        user_id: user_id,
      ...
      if (p) {
        ...
        if (p && p.access_token) {
          self.setAccessToken(p.access_token)
        }
        return p
      }
    }
...
regular).
  ...
  self.placeGTT = function (params) {
    const payload = self._getGTTPayload(params)

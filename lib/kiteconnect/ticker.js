...
const KiteTicker = function (params) {
  ...
  const read_timeout = 5 // seconds
  let reconnect_max_delay = 0
  let reconnect_max_tries = 0

  // message flags (outgoing)
  const mSubscribe = 'subscribe'
  const mUnSubscribe = 'unsubscribe'
  const mSetMode = 'mode'
  ...
  const mLogout = 12
  const mReload = 13
  const mClearCache = 14

  // public constants
  const modeFull = 'full' // Full quote including market depth. 164 bytes.
  const modeQuote = 'quote' // Quote excluding market depth. 52 bytes.
  const modeLTP = 'ltp'
  ...
  function parseBinary (binpacks) {
    ...
    for (let n = 0; n < packets.length; n++) {
      const bin = packets[n]
      const instrument_token = buf2long(bin.slice(0, 4))
      ...
      if (bin.byteLength === 8) {
        ...
        if (bin.byteLength === 184) {
          ...
          let s = 0; const depth = bin.slice(64, 184)
          for (let i = 0; i < 10; i++) {
            s = i * 12
  ...
  function splitPackets (bin) {
    // number of packets
    const num = buf2long(bin.slice(0, 2))
    let j = 2
    const packets = []
    ...
    for (let i = 0; i < num; i++) {
      ...
      const packet = bin.slice(j + 2, j + 2 + size)
